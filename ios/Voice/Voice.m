...
@property (nonatomic, strong) NSTimer *speechTimeoutTimer; // Timer to detect speech inactivity
...
//    if (self.speechTimeoutTimer != nil) { // Invalidate the timer
...
- (void) setupAndStartRecognizing:(NSString*)localeStr {
    ...
    if (audioSessionError != nil) {
        ...
        // Start the speech timeout timer
//        [self resetSpeechTimeoutTimer];
    }
    @catch (NSException *exception) {
    NSLog(@"[Error] - %@ %@", exception.name, exception.reason);
    [self sendResult:@{@"code": @"start_recording", @"message": [exception reason]} :nil :nil :nil];
...
// New method to reset the speech timeout timer
//- (void)resetSpeechTimeoutTimer {
//    if (self.speechTimeoutTimer != nil) {
//        [self.speechTimeoutTimer invalidate];
//        self.speechTimeoutTimer = nil;
//    }
//    
//    self.speechTimeoutTimer = [NSTimer scheduledTimerWithTimeInterval:5.0 // Adjust the timeout duration as needed
//                                                               target:self
//                                                             selector:@selector(handleSpeechTimeout)
//                                                             userInfo:nil
//                                                              repeats:NO];
//}

// New method to handle speech timeout
...
// Added to show alert when user disabled Permissions
...
// Custom sound of activating speach/microphone
//- (void)playStartSound {
//    NSString *soundPath = [[NSBundle mainBundle] pathForResource:@"start_sound" ofType:@"wav"];
        ...
        [SFSpeechRecognizer requestAuthorization:^(SFSpeechRecognizerAuthorizationStatus status) {
            dispatch_async(dispatch_get_main_queue(), ^{
                switch (status) {
                    case SFSpeechRecognizerAuthorizationStatusNotDetermined:
                        [self sendResult:RCTMakeError(@"Speech recognition not yet authorized", nil, nil) :nil :nil :nil];
                        [self showAlertWithTitle:@"Speech Recognition Not Authorized" message:@"Please enable speech recognition access in settings."];
                        break;
                    case SFSpeechRecognizerAuthorizationStatusDenied:
                        [self sendResult:RCTMakeError(@"User denied access to speech recognition", nil, nil) :nil :nil :nil];
                        [self showAlertWithTitle:@"Speech Recognition Access Denied" message:@"Please enable speech recognition access in settings."];
                        break;
                    case SFSpeechRecognizerAuthorizationStatusRestricted:
                        [self sendResult:RCTMakeError(@"Speech recognition restricted on this device", nil, nil) :nil :nil :nil];
                        [self showAlertWithTitle:@"Speech Recognition Restricted" message:@"Speech recognition is restricted on this device."];
                        break;
                    case SFSpeechRecognizerAuthorizationStatusAuthorized:
//                        [self playStartSound]; // Play custom sound
                        [self setupAndStartRecognizing:localeStr];
                        break;
                }
                callback(@[@false]);
